...
function ChatList({ onUserSelect }) {
  ...
  const { _id } = useAppselector((state: any) => state.user.value);
        ...
        >
          <Box
            ...
            sx={{
              ...
              width: "100%",
              borderBottom:
                index !== filteredChats.length - 1 ? "1px solid #ddd" : "none",
              padding: "15px 0px",
            }}
          >
            <Avatar
              src={chat?.profileImage || ""}
              sx={{ bgcolor: orangeText, height: "40px", width: "40px" }}
            >
              {chat?.first_name?.[0] || ""}
              {chat?.last_name?.[0] || ""}
            </Avatar>
            <Box sx={{ width: "100%" }}>
              ...
              >
                <Typography
                  variant="h4"
                  sx={{ color: orangeText, fontSize: "14px", lineHeight: "1" }}
                >
                  {chat?.first_name} {chat?.last_name}
                </Typography>
