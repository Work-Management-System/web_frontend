...
import { Search, Add as AddIcon, Cancel, Person, Folder, Info, Warning, CheckCircle, AccessTime, AddCircleOutline } from "@mui/icons-material";
...
const ConfirmDialog = ({ open, onClose, onConfirm }) => {
  return (
    <Dialog open={open} onClose={() => onClose()}>
      ...
      <DialogContent>
        <DialogContentText>
          You haven’t assigned this task to any user.
          Do you want to continue without assigning a user?
        </DialogContentText>
      </DialogContent>
      <DialogActions>
        <Button onClick={() => onClose()} variant="outlined" sx={{ color: "var(--primary-color-2)", borderColor: "var(--primary-color-2)" }}>
          No
        </Button>
        <Button onClick={() => { onConfirm(); onClose(); }} variant="contained" sx={{ backgroundColor: "var(--primary-color-1)", color: "#ffffff", "&:hover": { backgroundColor: "var(--primary-color-1-hover)" } }}>
          Yes
        </Button>
      </DialogActions>
    </Dialog>
...
const KanbanBoard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (selectedUser) {
      filtered = filtered.filter((report) => report.current_user.id === selectedUser);
    }
  ...
  }, [selectedUser, selectedPriority, selectedProject, selectedDepartment, searchTerm, reports]);
  ...
  const updateColumnTasks = (updatedReports: Report[]) => {
    const tasksByColumn = columns.reduce((acc, column) => {
      acc[column.id] = updatedReports.filter((report) => report.status === column.id);
      return acc;
...
const actuallyAddTask = async (values, { resetForm }) => {
    ...
    while (!isUnique && attempts < maxAttempts) {
      ...
      try {
        ...
        for (const file of pendingAttachments) {
          await axiosInstance.patch(`/task-maangement/add-attachment/${taskId}`, {
            ...file,
  ...
  const handleUpdateTask = async (values: any, { resetForm }: any) => {
    ...
    try {
      ...
      const updatedReports = reports.map((r) => (r.id === taskToEdit.id ? updatedTask : r));
      setFilteredReports(updatedReports);
      updateColumnTasks(updatedReports);
      handleCloseModal();
      setTaskId(null)
  ...
  return (
    <>
          ...
          >
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start" sx={{ mr: 0.5 }}>
                    <Search sx={{ color: '#5e6c84', fontSize: 18 }} /> {/* Reduced icon size */}
                  </InputAdornment>
                ),
              }}
              sx={{
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) =>
                option.id ? `${option.first_name} ${option.last_name}` : 'All Users'
              }
              value={
                selectedUser
                  ? usersWithTasks.find((user) => user.id === selectedUser) || null
                  : null
              }
              onChange={(_, newValue) => {
            ...
            <FormControl sx={{ minWidth: 120 }} size="small">
              <InputLabel sx={{ color: '#5e6c84', fontSize: '0.8rem' }}>Priority</InputLabel>
              <Select
              ...
              >
                ...
                <MenuItem value="p1" sx={{ fontSize: '0.8rem' }}>P1</MenuItem>
                <MenuItem value="p2" sx={{ fontSize: '0.8rem' }}>P2</MenuItem>
                <MenuItem value="p3" sx={{ fontSize: '0.8rem' }}>P3</MenuItem>
              </Select>
            </FormControl>
            <Autocomplete
              options={projects}
              getOptionLabel={(option) => option.title}
              value={projects.find((project) => project.id === selectedProject) || null}
              ...
              <FormControl sx={{ minWidth: 130 }} size="small">
                ...
                >
                  ...
                  {departmentOptions.map((dept) => (
                    <MenuItem key={dept.id} value={dept.name} sx={{ fontSize: '0.8rem' }}>
                      {dept.name}
                    </MenuItem>
                  ))}
                </Select>
              ...
              <FormControl size="small" sx={{ minWidth: 130 }}>
                <Select
                  ...
                  sx={{
                    ...
                    borderRadius: '8px',
                    '& .MuiOutlinedInput-notchedOutline': { borderColor: 'var(--primary-color-1)' },
                    '&:hover .MuiOutlinedInput-notchedOutline': { borderColor: 'var(--primary-color-1)' },
                    '& .MuiSelect-icon': { color: 'var(--primary-color-1)' },
                  }}
                >
                  <MenuItem value="my" sx={{ fontSize: '0.8rem' }}>My Tasks</MenuItem>
                  <MenuItem value="team" sx={{ fontSize: '0.8rem' }}>My Team</MenuItem>
                  <MenuItem value="all" sx={{ fontSize: '0.8rem' }}>My Projects View</MenuItem>
                  {currentUserRole.priority > 2 &&
                  <MenuItem value="department" sx={{ fontSize: '0.8rem' }}>My Dept. View</MenuItem>
}
                </Select>
              </FormControl>
            ) : null}
      ...
      <Modal open={openModal} onClose={handleCloseModal} aria-labelledby="task-modal-title">
        <Box
          sx={{
            position: 'absolute',
            top: '50%',
        ...
        >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Box>
                  ...
                  <Typography sx={{ opacity: 0.9, fontSize: '0.75rem' }}>
                    {taskEditMode ? 'Update task details' : 'Fill in task details'}
                  </Typography>
                </Box>
              </Box>
              <IconButton
          ...
          {error && (
            ...
            >
              ...
              <Typography sx={{ color: '#dc2626', fontSize: '0.75rem' }}>{error}</Typography>
            </Box>
          )}
          ...
          >
            {({ values, setFieldValue, errors, touched, dirty, isSubmitting }) => (
                ...
                <Box sx={{ p: 1.5, flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                          <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--primary-color-1)' }}>
                            <RequiredLabel label="Task Title" />
                          </Typography>
                          <Tooltip title="Enter a clear and descriptive title">
                        ...
                        <Field
                          ...
                          sx={{
                            '& .MuiOutlinedInput-root': {
                              ...
                              height: 36,
                              fontSize: '0.8rem',
                              '&:hover fieldset': { borderColor: 'var(--primary-color-2)' },
                            },
                      ...
                      <Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                          ...
                          <Tooltip title="Add detailed task information">
                            <Info sx={{ fontSize: '1rem', color: '#6b7280' }} />
                          </Tooltip>
                        </Box>
                        <EditorContainer sx={{
                          width: '790px', display: 'flex', flexDirection: 'column', padding: 0,
 }}>
                          <Tiptap
                            initialContent={values.description}
                            onSave={(content) => setFieldValue('description', content)}
                            onPendingAttachment={(fileData) => setPendingAttachments((prev) => [...prev, fileData])}
                          />
                          </EditorContainer>
                      </Box>
                      {/* Subtasks Section */}
                        ...
                        <FieldArray name="subtasks">
                          {({ remove, push }) => (
                            ...
                            >
                              ...
                              >
                                {[...values.subtasks].reverse().map((subtask, index) => {
                                  const originalIndex = values.subtasks.length - 1 - index; // Calculate original index
                                  return (
                                    <Paper
                                      key={originalIndex}
                                      sx={{
                    ...
                    >
                      ...
                      <Paper sx={{ p: 1.5, borderRadius: 1, border: '1px solid #e2e8f0' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, minWidth: 100 }}>
                            <Folder sx={{ color: 'var(--primary-color-2)', fontSize: '1.1rem' }} />
                          ...
                          >
                            <Autocomplete
                              ...
                              value={projects.find((p) => p.id === values.project_id) || null}
                              renderInput={(params) => (
                                <TextField
                                  {...params}
                      ...
                      {!taskEditMode && (
                        <Paper sx={{ p: 1.5, borderRadius: 1, border: '1px solid #e2e8f0' }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, minWidth: 100 }}>
                              <Person sx={{ color: 'var(--primary-color-2)', fontSize: '1.1rem' }} />
                              <Typography sx={{ fontSize: '0.85rem', fontWeight: 600 }}>
                            ...
                            >
                              <Autocomplete
                                ...
                                value={modalUsers.find((u) => u.id === values.current_user_id) || null}
                                onChange={(_, selectedUser) => {
                                  setFieldValue('current_user_id', selectedUser?.id || '');
                                }}
                                disabled={!values.project_id}
                                size="small"
                                noOptionsText={values.project_id ? 'No team members' : 'Select project first'}
                                renderInput={(params) => {
                                  const selectedUser = modalUsers.find((u) => u.id === values.current_user_id);
                      ...
                      <Paper sx={{ p: 1.5, borderRadius: 1, border: '1px solid #e2e8f0' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, minWidth: 120 }}>
                            <Warning sx={{ color: 'var(--primary-color-2)', fontSize: '1.1rem' }} />
                            <Typography sx={{ fontSize: '0.85rem', fontWeight: 600 }}>
                              Priority Level
                            </Typography>
                          </Box>
                          <Box sx={{ display: 'flex', gap: 1.2, flex: 1 }}>
                            {[
                              { value: 'p1', label: 'Critical', color: '#ef4444' },
                              { value: 'p2', label: 'High', color: '#f59e0b' },
                              { value: 'p3', label: 'Normal', color: '#10b981' },
                            ].map((priority) => (
                              <Box
                                key={priority.value}
                                ...
                                sx={{
                                  ...
                                  p: 1,
                                  borderRadius: 1,
                                  border: `1.5px solid ${values.priority === priority.value ? priority.color : '#e2e8f0'}`,
                                  backgroundColor: values.priority === priority.value ? `${priority.color}10` : '#f8fafc',
                                  cursor: 'pointer',
                                  transition: 'all 0.2s',
                                  '&:hover': {
                              ...
                              >
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  <Typography
                                    fontWeight={600}
                                    fontSize="0.75rem"
                                    color={values.priority === priority.value ? priority.color : 'inherit'}
                                  >
                                    {priority.label}
                                  </Typography>
                                  {values.priority === priority.value && (
                                    <CheckCircle sx={{ color: priority.color, fontSize: '0.9rem' }} />
                                  )}
                                </Box>
                              </Box>
                            ))}
                      ...
                      <Paper sx={{ p: 1.5, borderRadius: 1, border: '1px solid #e2e8f0' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, minWidth: 120 }}>
                            <AccessTime sx={{ color: 'var(--primary-color-2)', fontSize: '1.1rem' }} />
                            <Typography sx={{ fontSize: '0.85rem', fontWeight: 600 }}>
                              Time Estimation
                            </Typography>
                          </Box>
                          <Box sx={{ display: 'flex', gap: 1.5, flex: 1 }}>
                            <Box sx={{ flex: 1 }}>
                              <Typography variant="caption" fontWeight={500} color="text.secondary" fontSize="0.7rem">
                                Days
                              </Typography>
                              <Field
                                as={TextField}
                                ...
                                onChange={(e) => {
                                  ...
                                  const totalMinutes =
                                    (parseFloat(days) || 0) * 1440 + (parseFloat(values.deadline_hours) || 0) * 60;
                            ...
                            <Box sx={{ flex: 1 }}>
                              ...
                              <Field
                                ...
                                onChange={(e) => {
                                  ...
                                  const totalMinutes =
                                    (parseFloat(values.deadline_days) || 0) * 1440 + (parseFloat(hours) || 0) * 60;
                                  setFieldValue('deadline_total_minutes', Math.round(totalMinutes).toString());
                                }}
                              />
                            </Box>
                        ...
                        <Box sx={{ mt: 1, p: 0.5, backgroundColor: '#f1f5f9', borderRadius: 1 }}>
                          ...
                          <Typography
                            ...
                            fontSize="0.75rem"
                          >
                            {Math.floor(parseFloat(values.deadline_total_minutes || '0') / 60)}h{' '}
                      ...
                      >
                        <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                          ...
                          >
                            {isSubmitting ? 'Processing...' : taskEditMode ? 'Update Task' : 'Create Task'}
                          </Button>
                        </Box>
                      </Box>
                    </Box>
