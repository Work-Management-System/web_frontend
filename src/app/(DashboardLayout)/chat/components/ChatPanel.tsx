...
export default function ChatPanel({
  ...
  const [composerEmojiAnchor, setComposerEmojiAnchor] =
  ...
  const getHeaderTitle = () => {
    ...
    if (isDM && currentSpace.otherMember) {
      return (
        `${currentSpace.otherMember.first_name || ""} ${currentSpace.otherMember.last_name || ""}`.trim() ||
        "Chat"
      );
    }
  ...
  useEffect(() => {
    if (spaceId) {
      ...
      dispatch(fetchMessages({ spaceId, limit: 50 }) as any)
        .then((result: any) => {
        ...
        try {
          ...
          axiosInstance
            ...
            .then(() => {
              ...
              console.log(
                `Read status updated for space ${spaceId}, resetting unread count`,
              );
  ...
  const handleScroll = useCallback(() => {
    ...
    if (scrollTop < 200) {
      ...
      if (hasMore && currentMessages.length > 0) {
        ...
        dispatch(
          fetchMessages({ spaceId, before: oldestMessage.id, limit: 50 }),
        );
      }
    }
  ...
  let currentGroup: { messages: Message[]; sender: any; date: string } | null =
    null;
  ...
  messages.forEach((msg) => {
    ...
    const timeDiff = currentGroup
      ? dayjs(msg.created_at).diff(
          dayjs(currentGroup.messages[0].created_at),
          "minute",
        )
      : Infinity;
  ...
  return (
    ...
    >
          ...
          <Box>
            ...
            <Typography variant="caption" sx={{ color: "#8e8e8e" }}>
              {isDM
                ...
                : `${currentSpace?.memberCount || 0} members`}
            </Typography>
          </Box>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          ...
          <Tooltip title={showInfoPanel ? "Hide info" : "Show info"}>
            <IconButton
              ...
              sx={{ color: showInfoPanel ? "#1a73e8" : "#666" }}
            >
              <InfoIcon />
            </IconButton>
          </Tooltip>
      ...
      >
          ...
          groupedMessages.map((group, groupIndex) => {
            ...
            const showDateSeparator =
              ...
              dayjs(group.date).format("YYYY-MM-DD") !==
                dayjs(groupedMessages[groupIndex - 1].date).format(
                  "YYYY-MM-DD",
                );
                      ...
                      >
                        {showName && (
                          ...
                          >
                            ...
                            >
                              {group.sender?.first_name}{" "}
                              {group.sender?.last_name}
                            </Typography>
                            {/* Optional tag badges */}
                            {msg.metadata?.tags && (
                              <Box sx={{ display: "flex", gap: 0.5 }}>
                                {msg.metadata.tags.map(
                          ...
                          {msg.parent_message_id &&
                            (() => {
                              ...
                              const replyToName = isReplyToSelf
                                ? "You"
                                : `${parentMsg?.sender?.first_name || ""} ${parentMsg?.sender?.last_name || ""}`.trim() ||
                                  "Unknown";
        ...
        {typingUsers.size > 0 && (
          <Box
            sx={{ display: "flex", alignItems: "center", gap: 1, px: 1, py: 1 }}
          >
            <Box
              sx={{
                display: "flex",
      ...
      >
        <MenuItem
          onClick={() => handleReply(messageMenuAnchor!.message)}
        ...
        {messageMenuAnchor?.message.sender_id === currentUser?.id && (
          <MenuItem
            onClick={() => handleEdit(messageMenuAnchor!.message)}
            sx={{ fontSize: "0.875rem" }}
          >
      ...
      >
        <Typography
          variant="caption"
          sx={{ fontWeight: 600, color: "#666", mb: 1, display: "block" }}
        >
          Choose a reaction
        </Typography>
        <Box
          sx={{ display: "flex", gap: 0.5, flexWrap: "wrap", maxWidth: 280 }}
        >
          {REACTION_EMOJIS.map((emoji) => (
            <IconButton
              ...
              onClick={() =>
                emojiPickerAnchor &&
                handleAddReaction(emojiPickerAnchor.message, emoji)
              }
              sx={{
                fontSize: "1.5rem",
                width: 44,
      ...
      >
        ...
        <Box
          sx={{ display: "flex", gap: 0.25, flexWrap: "wrap", maxWidth: 320 }}
        >
