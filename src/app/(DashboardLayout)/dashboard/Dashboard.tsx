...
export default function Dashboard() {
  ...
  const [totalSubscriptionsCount, setTotalSubscriptionsCount] =
    useState<number>(0);
  const [activeSubscriptionsCount, setActiveSubscriptionsCount] =
    useState<number>(0);
  const [monthlyRevenue, setMonthlyRevenue] = useState<number>(0);
  const [newTenantsThisMonth, setNewTenantsThisMonth] = useState<number>(0);
  const [newSubscriptionsThisMonth, setNewSubscriptionsThisMonth] =
    useState<number>(0);
    ...
    try {
      ...
      const count = Array.isArray(response.data)
        ? response.data.reduce(
            (sum: number, user: any) => sum + (user.missedReports?.length || 0),
            0,
          )
        : 0;
    ...
    try {
      if (userPriority === 1) {
        ...
        const response = await axiosInstance.get(
          `/project-management/user-projects/${user?.id}`,
        );
  ...
  const fetchTeamUsersCount = async () => {
    ...
    try {
      if (userPriority === 2) {
        ...
        // Manager: get team members (subordinates including manager)
        const response = await axiosInstance.get(`/user/team-list/${user?.id}`);
        setTeamUsersCount(response.data?.data?.length || 0);
      } else {
        // Employee (priority 4): get team under their manager (manager + reportees including self)
        let managerId =
          (user as any)?.reporting_manager_id || user?.reporting_manager;
        if (!managerId && user?.id) {
          try {
            const profileRes = await axiosInstance.get(
              `/user/find-one/${user.id}`,
            );
            managerId = profileRes.data?.data?.reporting_manager_id;
          } catch {
            managerId = null;
          }
        }
  ...
  const handleClockInOut = async () => {
    try {
      if (!attendanceStatus) {
        ...
        attendanceStatus.status === "ON_LUNCH"
      ) {
        // Clock Out
    ...
    try {
      ...
      if (analytics) {
        ...
        if (analytics.subscriptionStatusCounts) {
          const total =
            analytics.subscriptionStatusCounts.active +
        ...
        if (
          ...
          const currentMonthRevenue = analytics.revenueAnalytics.find(
            (r: any) =>
          ...
          if (currentMonthRevenue) {
            ...
            const totalRevenue = analytics.revenueAnalytics.reduce(
              (sum: number, r: any) => {
        ...
        if (analytics.onboardingTrend) {
          const currentMonthLabel = format(new Date(), "MMM yyyy");
          const currentMonthData = analytics.onboardingTrend.find(
            (t: any) =>
              t.period.toLowerCase() === currentMonthLabel.toLowerCase(),
          );
          if (currentMonthData) {
            // Approximate new subscriptions - most new tenants have subscriptions
            setNewSubscriptionsThisMonth(
              Math.floor(currentMonthData.count * 0.9),
            );
          }
        }
      }
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const mappedReports: Report[] = uniqueTickets?.map((ticket: any) => ({
        ...
        current_user: {
          ...
          first_name: ticket.current_user?.first_name
            ? ticket.current_user?.first_name
            : "",
          last_name: ticket.current_user?.last_name
            ? ticket.current_user?.last_name
            : "",
        },
      ...
      if (userPriority === 1) {
        ...
        const adminTasksCount = mappedReports.filter(
          (r) => !["completed", "testable"].includes(r.status?.toLowerCase()),
        ).length;
        ...
        const userTasks = mappedReports.filter(
          (r) =>
  ...
  const onboardingChartOptions: ApexOptions = {
    ...
    xaxis: {
      categories: analytics?.onboardingTrend?.map((item) => item.period) || [],
    },
  ...
  const revenueChartOptions: ApexOptions = {
    ...
    dataLabels: { enabled: false },
    xaxis: {
      categories: analytics?.revenueAnalytics?.map((item) => item.period) || [],
    },
  ...
  const revenueChartSeries = [
    {
      name: "Revenue (₹)",
      data:
        analytics?.revenueAnalytics?.map((item) => Number(item.revenue)) || [],
    },
  ];
  ...
  const tenantStatusChartOptions: ApexOptions = {
    ...
    series: analytics?.statusCounts
      ? [
          analytics.statusCounts.active,
          analytics.statusCounts.inactive,
          analytics.statusCounts.suspended,
        ]
      : [],
    colors: [
  ...
  const subscriptionStatusChartOptions: ApexOptions = {
    ...
    legend: { position: "bottom", labels: { colors: [chartColors.textColor] } },
    title: {
      text: "Subscription Status Counts",
      align: "left",
      style: {
        color: chartColors.textColor,
        fontFamily: "inherit",
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      value={groupBy}
                      onChange={(e) =>
                        setGroupBy(e.target.value as "month" | "day")
                      }
            ...
            <>
              ...
              <Box data-tour="dashboard-quick-access">
                <CategoryCards
                  ...
                  categories={[
                    {
                      name: userPriority === 2 ? "Employees" : "Team",
              ...
              <Box
                sx={{ mb: 3, width: "100%" }}
                data-tour="dashboard-attendance-sidebar"
              >
                <DashboardSidebar
                  onUserClick={(id) => router.push(`/users/${id}`)}
                />
                ...
                >
                  ...
                  >
                    ...
                    <Box sx={{ flex: 1 }} data-tour="project-overview">
                      <ProjectOverview
                        projects={analytics?.allProjectDetails}
                      />
                    </Box>
                  </Box>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      {analytics?.usersWithNoActiveTasks && (
                        <div className="bg-white/50 p-6 rounded-2xl shadow-xl border border-gray-200">
                          <h6 className="font-bold text-gray-800 mb-6">
                            👥 Users with No Active Tasks
                          </h6>
                          <div className="overflow-x-auto">
                            <div className="max-h-80 overflow-y-auto rounded-xl shadow-sm">
                              <table className="min-w-full">
                                <thead
                                  style={{
                                    backgroundColor: chartColors.primaryBgColor,
                                  }}
                                >
                                  <tr>
                                    <th
                                ...
                                <tbody className="divide-y divide-gray-100">
                                  {analytics.usersWithNoActiveTasks?.map(
                                    (user) => (
                                      <tr
                                        key={user.id}
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${
                              report.taskReports_status === "completed"
...
const UserTasks = ({ users, reports, columns }) => {
  ...
  const userTasks = useMemo(() => {
    ...
    reports
      ?.filter((report) =>
        ["pending", "in_progress", "on_hold"].includes(report.status),
      )
      .forEach((report) => {
  ...
  const getUserColor = (p1Count, p2Count, p3Count) => {
    ...
    if (score === 0)
      return "rgba(200, 200, 200, 0.3)"; // Gray: No tasks
    else if (score >= 1 && score <= 3)
      return "rgba(153, 255, 153, 0.3)"; // Green: Light work
    else if (score >= 4 && score <= 6)
      return "rgba(255, 255, 153, 0.3)"; // Yellow: Medium load
  ...
  return (
    <div
      className="bg-white/50 p-3 rounded-2xl mb-6"
    ...
    >
      ...
      >
        <Typography
          variant="h6"
          sx={{ fontWeight: "semibold", color: "#172b4d" }}
        >
          User Task Overview
        </Typography>
        {userPriority !== 4 && (
        ...
        >
          ...
          <MenuItem onClick={() => handleSortSelect("totalTasks")}>
            Sort by Total Tasks
          </MenuItem>
          <MenuItem onClick={() => handleSortSelect("p1Tasks")}>
            Sort by P1 Tasks
          </MenuItem>
          <MenuItem onClick={() => handleSortSelect("p2Tasks")}>
            Sort by P2 Tasks
          </MenuItem>
          <MenuItem onClick={() => handleSortSelect("p3Tasks")}>
            Sort by P3 Tasks
          </MenuItem>
        </Menu>
      </Box>
      <div className="p-2 rounded-2xl mb-6 max-h-[68vh] overflow-y-auto">
        ...
        >
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
            <Typography sx={{ fontWeight: "bold", color: "#172b4d", flex: 2 }}>
              User
            </Typography>
            <Typography
              sx={{
                ...
                >
                  <AccordionSummary
                    ...
                    sx={{
                      ...
                      transition:
                        "border-radius 0.3s ease, min-height 0.3s ease",
                      "&:not(.Mui-expanded)": {
                        borderRadius: "50px",
                        minHeight: "40px",
                      },
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography
                        sx={{ flex: 2, color: "#172b4d", fontSize: "0.9rem" }}
                      >
                        {user.first_name} {user.last_name}
                      </Typography>
                      <Typography
                    ...
                    >
                      ...
                      >
                        {[
                          ...
                          let gradientColor =
                            "linear-gradient(135deg, #dc2626 0%, #e57373 100%)"; // Softer red for P1
                          if (priority === "p2")
                            gradientColor =
                              "linear-gradient(135deg, #d97706 0%, #f4c261 100%)"; // Muted amber for P2
                          if (priority === "p3")
                            gradientColor =
                              "linear-gradient(135deg, #059669 0%, #6ccab5 100%)"; // Subdued green for P3
